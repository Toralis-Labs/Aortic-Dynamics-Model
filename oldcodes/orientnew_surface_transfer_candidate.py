...
def summarize_surface_label_transfer(
    ...
    touches_trunk = {
        "right_renal": bool(contact_map.get(LABEL_RIGHT_RENAL, {}).get(LABEL_AORTA_TRUNK, 0) > 0),
        "left_renal": bool(contact_map.get(LABEL_LEFT_RENAL, {}).get(LABEL_AORTA_TRUNK, 0) > 0),
        "right_main_iliac": bool(contact_map.get(LABEL_RIGHT_ILIAC, {}).get(LABEL_AORTA_TRUNK, 0) > 0),
        "left_main_iliac": bool(contact_map.get(LABEL_LEFT_ILIAC, {}).get(LABEL_AORTA_TRUNK, 0) > 0),
    }

    allowed_surface_names = [
        LABEL_ID_TO_NAME[LABEL_AORTA_TRUNK],
        LABEL_ID_TO_NAME[LABEL_RIGHT_RENAL],
        LABEL_ID_TO_NAME[LABEL_LEFT_RENAL],
        LABEL_ID_TO_NAME[LABEL_OTHER],
    ]
    present_nonzero = [str(name) for name, count in cell_counts.items() if int(count) > 0]
    validation = {
        "abdominal_aorta_trunk_exists": bool(cell_counts.get(LABEL_ID_TO_NAME[LABEL_AORTA_TRUNK], 0) > 0),
        "right_renal_exists": bool(cell_counts.get(LABEL_ID_TO_NAME[LABEL_RIGHT_RENAL], 0) > 0),
        "left_renal_exists": bool(cell_counts.get(LABEL_ID_TO_NAME[LABEL_LEFT_RENAL], 0) > 0),
        "abdominal_aorta_trunk_component_count": int(component_counts.get(LABEL_ID_TO_NAME[LABEL_AORTA_TRUNK], 0)),
        "right_renal_component_count": int(component_counts.get(LABEL_ID_TO_NAME[LABEL_RIGHT_RENAL], 0)),
        "left_renal_component_count": int(component_counts.get(LABEL_ID_TO_NAME[LABEL_LEFT_RENAL], 0)),
        "abdominal_aorta_trunk_single_component": bool(component_counts.get(LABEL_ID_TO_NAME[LABEL_AORTA_TRUNK], 0) == 1),
        "right_renal_single_component": bool(component_counts.get(LABEL_ID_TO_NAME[LABEL_RIGHT_RENAL], 0) == 1),
        "left_renal_single_component": bool(component_counts.get(LABEL_ID_TO_NAME[LABEL_LEFT_RENAL], 0) == 1),
        "right_renal_touches_trunk": bool(touches_trunk["right_renal"]),
        "left_renal_touches_trunk": bool(touches_trunk["left_renal"]),
        "right_main_iliac_surface_count": int(cell_counts.get(LABEL_ID_TO_NAME[LABEL_RIGHT_ILIAC], 0)),
        "left_main_iliac_surface_count": int(cell_counts.get(LABEL_ID_TO_NAME[LABEL_LEFT_ILIAC], 0)),
        "right_main_iliac_absent_from_surface": bool(cell_counts.get(LABEL_ID_TO_NAME[LABEL_RIGHT_ILIAC], 0) == 0),
        "left_main_iliac_absent_from_surface": bool(cell_counts.get(LABEL_ID_TO_NAME[LABEL_LEFT_ILIAC], 0) == 0),
        "non_target_surface_labels_present": [str(name) for name in present_nonzero if str(name) not in allowed_surface_names],
    }

    return {
        "active_surface_labels": list(allowed_surface_names),
        "forbidden_surface_labels": [
            LABEL_ID_TO_NAME[LABEL_RIGHT_ILIAC],
            LABEL_ID_TO_NAME[LABEL_LEFT_ILIAC],
        ],
        "cell_counts": cell_counts,
        "point_counts": point_counts,
        "component_counts": component_counts,
        "area_by_label": area_by_label,
        "touches_trunk": touches_trunk,
...
def choose_majority_neighbor_label(labels: np.ndarray, adjacency: List[List[int]], cell_id: int) -> int:
    ...
    best_label = min(counts.keys(), key=lambda lid: (-counts[lid], priority.get(int(lid), 99), int(lid)))
...
def derive_surface_point_labels(pd: "vtkPolyData", cell_labels: np.ndarray) -> np.ndarray:
    ...
    for pid, cells in enumerate(point_to_cells):
        ...
        for ci in cells:
            ...
            counts[lid] = int(counts.get(lid, 0) + 1)
        best = min(counts.keys(), key=lambda lid: (-counts[lid], priority.get(int(lid), 99), int(lid)))
...
def build_surface_label_transfer(
    ...
    for lid in [LABEL_AORTA_TRUNK, LABEL_RIGHT_RENAL, LABEL_LEFT_RENAL]:
    ...
    missing_targets = [
        LABEL_ID_TO_NAME[int(lid)]
        for lid in [LABEL_AORTA_TRUNK, LABEL_RIGHT_RENAL, LABEL_LEFT_RENAL]
        if int(lid) not in distance_sq
    ]
    if missing_targets:
        work_warnings.append(
            f"W_SURFACE_TARGETS_MISSING: surface transfer missing centerline seeds for {', '.join(missing_targets)}."
        )

    if not distance_sq:
        work_warnings.append("W_SURFACE_NO_TARGET_SEEDS: no trunk or renal centerline segments available for surface transfer.")
        cell_labels = np.zeros((n_cells,), dtype=int)
        point_labels = derive_surface_point_labels(surface_pd, cell_labels)
        summary = summarize_surface_label_transfer(
            cell_labels=cell_labels,
    ...
    if trunk_bank and trunk_inlet is None:
        longest_trunk = get_longest_surface_bank_branch(trunk_bank)
        if longest_trunk is not None:
            trunk_pts = np.asarray(longest_trunk.get("points", np.zeros((0, 3), dtype=float)), dtype=float)
            if trunk_pts.shape[0] > 0:
                trunk_inlet = {
                    "point": trunk_pts[0].astype(float),
                    "tangent": unit(trunk_pts[min(1, trunk_pts.shape[0] - 1)] - trunk_pts[0]) if trunk_pts.shape[0] > 1 else np.zeros((3,), dtype=float),
                }
                work_warnings.append("W_SURFACE_INLET_FALLBACK: trunk inlet landmark missing; used proximal trunk endpoint for surface seeding.")
    if trunk_bank and trunk_bif is None:
        ...
        if longest_trunk is not None:
            ...
            if trunk_pts.shape[0] > 0:
                trunk_bif = {
                    "point": trunk_pts[-1].astype(float),
                    "tangent": unit(trunk_pts[-1] - trunk_pts[max(0, trunk_pts.shape[0] - 2)]) if trunk_pts.shape[0] > 1 else np.zeros((3,), dtype=float),
                }
                work_warnings.append(
                    "W_SURFACE_BIF_FALLBACK: trunk bifurcation landmark missing; used distal trunk endpoint for bifurcation clipping."
                )

    distal_to_bif_mask = np.zeros((n_cells,), dtype=bool)
    bifurcation_plane_offset = 0.0
    if trunk_bif is not None and np.linalg.norm(np.asarray(trunk_bif.get("tangent", np.zeros((3,), dtype=float)), dtype=float)) > EPS:
        bif_point = np.asarray(trunk_bif["point"], dtype=float).reshape(3)
        bif_tangent = unit(np.asarray(trunk_bif["tangent"], dtype=float).reshape(3))
        distal_to_bif_mask = np.asarray(((cell_centers - bif_point[None, :]) @ bif_tangent) > bifurcation_plane_offset, dtype=bool)
    else:
        work_warnings.append("W_SURFACE_BIF_CLIP_UNAVAILABLE: unable to construct bifurcation clip plane; distal iliac surface may remain as other.")

    renal_region_masks: Dict[int, np.ndarray] = {
        LABEL_RIGHT_RENAL: np.zeros((n_cells,), dtype=bool),
        LABEL_LEFT_RENAL: np.zeros((n_cells,), dtype=bool),
    }
    renal_region_settings: Dict[str, Any] = {}
    trunk_distance_sq = distance_sq.get(LABEL_AORTA_TRUNK, np.full((n_cells,), float("inf"), dtype=float))
    renal_specs = [
        (LABEL_RIGHT_RENAL, "RightRenalOrigin", "right_renal"),
        (LABEL_LEFT_RENAL, "LeftRenalOrigin", "left_renal"),
    ]
    renal_trunk_guard_ratio = 1.10
    renal_distance_limit_scale = 2.40

    for lid, landmark_key, summary_key in renal_specs:
        bank = label_bank.get(int(lid))
        if bank is None or int(lid) not in distance_sq:
            renal_region_settings[summary_key] = {"assigned_cells": 0, "reason": "missing_centerline_branch"}
            continue

        renal_branch = get_longest_surface_bank_branch(bank)
        if renal_branch is None:
            renal_region_settings[summary_key] = {"assigned_cells": 0, "reason": "missing_landmark_and_branch"}
            work_warnings.append(f"W_SURFACE_{summary_key.upper()}_BRANCH_MISSING: renal branch geometry unavailable for {summary_key}.")
            continue
        branch_points = np.asarray(renal_branch.get("points", np.zeros((0, 3), dtype=float)), dtype=float)
        if branch_points.shape[0] == 0:
            renal_region_settings[summary_key] = {"assigned_cells": 0, "reason": "missing_landmark_and_branch"}
            work_warnings.append(f"W_SURFACE_{summary_key.upper()}_BRANCH_MISSING: renal branch geometry unavailable for {summary_key}.")
            continue
        branch_abscissa = np.asarray(renal_branch.get("abscissa", compute_abscissa(branch_points)), dtype=float)
        branch_length = float(renal_branch.get("length", branch_abscissa[-1] if branch_abscissa.size else 0.0))

        landmark_info = find_surface_bank_landmark(bank, landmark_key, tangent_mode="forward")
        if landmark_info is None:
            landmark_info = find_surface_bank_landmark(trunk_bank, landmark_key, tangent_mode="forward")
        if landmark_info is None:
            landmark_info = {
                "point": branch_points[0].astype(float),
            }
            work_warnings.append(f"W_SURFACE_{summary_key.upper()}_LANDMARK_FALLBACK: used proximal renal branch endpoint for takeoff point.")

        takeoff_point = np.asarray(landmark_info["point"], dtype=float).reshape(3)
        takeoff_tangent = unit(branch_points[1] - branch_points[0]) if branch_points.shape[0] > 1 else np.zeros((3,), dtype=float)
        if np.linalg.norm(takeoff_tangent) <= EPS:
            takeoff_tangent = unit(np.asarray(landmark_info.get("tangent", np.zeros((3,), dtype=float)), dtype=float).reshape(3))
        landmark_index = 0
        takeoff_s = 0.0
        local_step = 0.0
        if branch_points.shape[0] > 1:
            local_step = float(np.linalg.norm(branch_points[1] - branch_points[0]))
        remaining_length = max(0.0, branch_length - takeoff_s)
        seed_offset = min(max(1.5 * local_step, 0.08 * branch_length), 0.35 * remaining_length) if remaining_length > EPS else 0.0
        renal_seed_point = (
            polyline_point_at_abscissa(branch_points, takeoff_s + seed_offset) if branch_points.shape[0] else takeoff_point.copy()
        )

        axial_proj = np.sum((cell_centers - takeoff_point[None, :]) * takeoff_tangent[None, :], axis=1) if np.linalg.norm(takeoff_tangent) > EPS else np.zeros((n_cells,), dtype=float)
        seed_front_mask = np.asarray(axial_proj >= -0.25 * max(local_step, EPS), dtype=bool) if np.linalg.norm(takeoff_tangent) > EPS else np.ones((n_cells,), dtype=bool)
        renal_seed_cell = nearest_cell_to_point(cell_centers, renal_seed_point, allowed_mask=seed_front_mask)
        if renal_seed_cell < 0:
            renal_seed_cell = nearest_cell_to_point(cell_centers, renal_seed_point, allowed_mask=None)
        if renal_seed_cell < 0:
            renal_region_settings[summary_key] = {"assigned_cells": 0, "reason": "missing_surface_seed"}
            work_warnings.append(f"W_SURFACE_{summary_key.upper()}_SEED_FAILED: unable to place renal surface seed.")
            continue

        seed_cells[int(lid)] = int(renal_seed_cell)
        seed_radius = math.sqrt(max(float(distance_sq[int(lid)][int(renal_seed_cell)]), 0.0))
        renal_distance_limit = max(renal_distance_limit_scale * max(seed_radius, EPS), 1.25 * max(local_step, EPS))
        ...
        branch_facing_mask = np.asarray(axial_proj >= -takeoff_back_margin, dtype=bool) if np.linalg.norm(takeoff_tangent) > EPS else np.ones((n_cells,), dtype=bool)
        ostium_bridge_mask = np.asarray(
            np.sum((cell_centers - takeoff_point[None, :]) ** 2, axis=1) <= (ostium_radius * ostium_radius),
            dtype=bool,
        ) & branch_facing_mask
        trunk_guard_mask = np.asarray(distance_sq[int(lid)] <= ((renal_trunk_guard_ratio ** 2) * trunk_distance_sq), dtype=bool)
        candidate_mask = np.asarray(distance_sq[int(lid)] <= (renal_distance_limit * renal_distance_limit), dtype=bool)
        candidate_mask &= branch_facing_mask
        candidate_mask &= np.asarray(trunk_guard_mask | ostium_bridge_mask, dtype=bool)
        candidate_mask[int(renal_seed_cell)] = True

        renal_component = connected_component_from_seed(int(renal_seed_cell), candidate_mask, adjacency)
        renal_mask = np.zeros((n_cells,), dtype=bool)
        if renal_component:
            renal_mask[np.asarray(renal_component, dtype=int)] = True

        renal_region_masks[int(lid)] = renal_mask
        renal_region_settings[summary_key] = {
            "assigned_cells": int(np.count_nonzero(renal_mask)),
            "seed_cell": int(renal_seed_cell),
            "seed_radius": float(seed_radius),
            "distance_limit": float(renal_distance_limit),
            "ostium_radius": float(ostium_radius),
            "takeoff_back_margin": float(takeoff_back_margin),
        }

    renal_overlap = np.asarray(renal_region_masks[LABEL_RIGHT_RENAL] & renal_region_masks[LABEL_LEFT_RENAL], dtype=bool)
    if np.any(renal_overlap):
        overlap_ids = np.flatnonzero(renal_overlap)
        right_dist_sq = distance_sq.get(LABEL_RIGHT_RENAL, np.full((n_cells,), float("inf"), dtype=float))
        left_dist_sq = distance_sq.get(LABEL_LEFT_RENAL, np.full((n_cells,), float("inf"), dtype=float))
        for ci in overlap_ids.tolist():
            ci_i = int(ci)
            if float(right_dist_sq[ci_i]) <= float(left_dist_sq[ci_i]):
                renal_region_masks[LABEL_LEFT_RENAL][ci_i] = False
            else:
                renal_region_masks[LABEL_RIGHT_RENAL][ci_i] = False
        work_warnings.append(
            f"W_SURFACE_RENAL_OVERLAP_RESOLVED: resolved {int(np.count_nonzero(renal_overlap))} overlapping renal surface cells by nearest renal centerline."
        )

    cell_labels[renal_region_masks[LABEL_RIGHT_RENAL]] = LABEL_RIGHT_RENAL
    cell_labels[renal_region_masks[LABEL_LEFT_RENAL]] = LABEL_LEFT_RENAL

    remaining_mask = np.asarray(cell_labels == LABEL_OTHER, dtype=bool) & np.asarray(~distal_to_bif_mask, dtype=bool)
    trunk_component: List[int] = []
    if trunk_inlet is not None:
        inlet_seed_cell = nearest_cell_to_point(cell_centers, trunk_inlet["point"], allowed_mask=remaining_mask)
        if inlet_seed_cell < 0:
            inlet_seed_cell = nearest_cell_to_point(cell_centers, trunk_inlet["point"], allowed_mask=None)
        if inlet_seed_cell >= 0:
            if not bool(remaining_mask[int(inlet_seed_cell)]):
                remaining_mask[int(inlet_seed_cell)] = True
            seed_cells[LABEL_AORTA_TRUNK] = int(inlet_seed_cell)
            trunk_component = connected_component_from_seed(int(inlet_seed_cell), remaining_mask, adjacency)
        else:
            work_warnings.append("W_SURFACE_TRUNK_SEED_FAILED: unable to place inlet seed for trunk surface component.")
    else:
        work_warnings.append("W_SURFACE_TRUNK_INLET_MISSING: trunk inlet landmark unavailable for surface trunk component.")

    trunk_mask = np.zeros((n_cells,), dtype=bool)
    if trunk_component:
        trunk_mask[np.asarray(trunk_component, dtype=int)] = True
        trunk_mask &= np.asarray(cell_labels == LABEL_OTHER, dtype=bool)
        trunk_mask &= np.asarray(~distal_to_bif_mask, dtype=bool)
    ...
    summary_settings = {
        ...
        "surface_cell_count": int(n_cells),
        "surface_point_count": int(n_points),
        "renal_distance_limit_scale": float(renal_distance_limit_scale),
        "renal_trunk_guard_ratio": float(renal_trunk_guard_ratio),
        "bifurcation_plane_offset": float(bifurcation_plane_offset),
        "distal_to_bifurcation_cell_count": int(np.count_nonzero(distal_to_bif_mask)),
        "trunk_component_cell_count": int(np.count_nonzero(trunk_mask)),
        "renal_regions": renal_region_settings,
    }
    summary = summarize_surface_label_transfer(
        cell_labels=cell_labels,
        point_labels=point_labels,
    ...
    if not bool(validation.get("abdominal_aorta_trunk_exists", False)):
        work_warnings.append("W_SURFACE_ABDOMINAL_AORTA_TRUNK_EMPTY: no surface cells assigned to abdominal_aorta_trunk.")
    if not bool(validation.get("right_renal_exists", False)):
        work_warnings.append("W_SURFACE_RIGHT_RENAL_EMPTY: no surface cells assigned to right_renal.")
    if not bool(validation.get("left_renal_exists", False)):
        work_warnings.append("W_SURFACE_LEFT_RENAL_EMPTY: no surface cells assigned to left_renal.")
    if not bool(validation.get("abdominal_aorta_trunk_single_component", False)):
        work_warnings.append(
            f"W_SURFACE_ABDOMINAL_AORTA_TRUNK_COMPONENTS: expected 1 connected component for abdominal_aorta_trunk, got {validation.get('abdominal_aorta_trunk_component_count', 0)}."
        )
    if not bool(validation.get("right_renal_single_component", False)):
        work_warnings.append(
            f"W_SURFACE_RIGHT_RENAL_COMPONENTS: expected 1 connected component for right_renal, got {validation.get('right_renal_component_count', 0)}."
        )
    if not bool(validation.get("left_renal_single_component", False)):
        work_warnings.append(
            f"W_SURFACE_LEFT_RENAL_COMPONENTS: expected 1 connected component for left_renal, got {validation.get('left_renal_component_count', 0)}."
        )
    if not bool(validation.get("right_renal_touches_trunk", False)):
        work_warnings.append("W_SURFACE_RIGHT_RENAL_DETACHED: labeled right_renal surface region does not touch trunk.")
    if not bool(validation.get("left_renal_touches_trunk", False)):
        work_warnings.append("W_SURFACE_LEFT_RENAL_DETACHED: labeled left_renal surface region does not touch trunk.")
    if int(validation.get("right_main_iliac_surface_count", 0)) != 0:
        work_warnings.append(
            f"W_SURFACE_RIGHT_MAIN_ILIAC_PRESENT: expected 0 surface cells for right_main_iliac, got {validation.get('right_main_iliac_surface_count', 0)}."
        )
    if int(validation.get("left_main_iliac_surface_count", 0)) != 0:
        work_warnings.append(
            f"W_SURFACE_LEFT_MAIN_ILIAC_PRESENT: expected 0 surface cells for left_main_iliac, got {validation.get('left_main_iliac_surface_count', 0)}."
        )
...
def main() -> int:
    ...
    try:
        ...
        centerlines, _ = compute_centerlines_vmtk(surface_tri, inlet_term.center, term_centers, warnings)
